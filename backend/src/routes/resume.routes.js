...
router.post("/parse", upload.single("resume"), async (req, res) => {
  try {
    ...
    commonSkills.forEach(skill => {
      // Use word boundaries to avoid partial matches where possible. E.g., for "Go" or "C++"
      const escapeRegExp = (string) => string.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const regex = new RegExp(`\\b${escapeRegExp(skill)}\\b`, 'i');
      
      if (regex.test(text) || lowerText.includes(skill.toLowerCase())) {
         // for skills with special characters like C++, Word boundary \b doesn't work well
         // so we fallback to simple includes, but to prevent false positives like 'go' in 'good',
         // we check regex for simple alphanumeric words, and includes for special char ones
